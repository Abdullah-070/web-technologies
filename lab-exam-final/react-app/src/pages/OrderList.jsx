              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ backgroundColor: '#f8fafc', p: 3, borderRadius: 2 }}>
                  {selectedOrder.items.map((item, idx) => (
                    <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', mb: 2.5, pb: 2, borderBottom: idx < selectedOrder.items.length - 1 ? '1px solid #e0e0e0' : 'none' }}>
                      <Typography variant="body2" sx={{ fontSize: '0.95rem' }}>{item.product?.name} x {item.quantity}</Typography>
                      <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.95rem' }}>${(item.price * item.quantity).toFixed(2)}</Typography>
                    </Box>
                  ))}
                  <Divider sx={{ my: 2.5 }} />
